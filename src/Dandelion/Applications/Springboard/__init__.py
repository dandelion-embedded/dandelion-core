...
class Springboard(DandelionApp):
    def __init__(self, manifest: DandelionAppManifest):
        ...
        self.container.set_style_local_bg_color(
            lv.cont.PART.MAIN, lv.STATE.DEFAULT, lv.color_hex(0x0A0A0A)
        )
        ...
        self.tabview.set_size(self.container.get_width(), self.container.get_height())
        self.tabview.align(self.container, lv.ALIGN.IN_TOP_LEFT, 0, 0)
        self.tabview.set_style_local_bg_opa(
            lv.tabview.PART.BG, lv.STATE.DEFAULT, lv.OPA._0
        )
        ...
        for i in range(self.numberOfPages):
            ...
            page.set_style_local_pad_all(lv.page.PART.SCROLLABLE, lv.STATE.DEFAULT, 0)
            c = lv.cont(page)
            c.set_fit(lv.FIT.PARENT)
            c.set_click(False)
            c.set_style_local_bg_opa(lv.tabview.PART.BG, lv.STATE.DEFAULT, lv.OPA._0)
            c.set_layout(lv.LAYOUT.GRID)

            pad = (c.get_width() - (80 * self.numberOfColumns)) / (
                self.numberOfColumns + 1
            )
            c.set_style_local_pad_left(lv.cont.PART.MAIN, lv.STATE.DEFAULT, int(pad))
            c.set_style_local_pad_right(lv.cont.PART.MAIN, lv.STATE.DEFAULT, int(pad))
            c.set_style_local_pad_inner(lv.cont.PART.MAIN, lv.STATE.DEFAULT, int(pad))
...
class SpringboardIcon(lv.cont):
    def __init__(self, parent, manifest: DandelionAppManifest):
        ...
        self.set_size(80, 80)
        self.set_style_local_bg_opa(self.PART.MAIN, lv.STATE.DEFAULT, lv.OPA._0)
        ...
        img_dsc = lv.img_dsc_t(
            {
                "header": {"always_zero": 0, "w": 180, "h": 180, "cf": lv.img.CF.TRUE_COLOR_ALPHA},
